...
- **Race a Friend**: host a room (pick a challenge first, then "Host a Race") and share the 5-character code, or join one — both players' engines start at the same synchronized instant and race live, with the opponent's kills/secrets/time shown in a second bar under your own. Each racer runs a fully independent copy of the game (their own device, their own instance), so nothing about a race changes how a solo run is judged or recorded — the loser's own attempt still resolves normally and can still be a personal best. First to *validly* finish (still passing the category's rules) wins. The relay is a small Azure Function (`api/negotiate`) that hands out a room-scoped access token for Azure Web PubSub; it only ever carries small progress/result messages, never game state.
