...
Derived from [doom-assist](https://github.com/PatAltimore/doom-assist): same engine, same twin-stick touch controls, keyboard play, and wired/Bluetooth gamepad support, with the assist-mode sidebar (map, cheats, code links) replaced by a run timer and a challenge board.
...
- **Gamepad**: a wired or Bluetooth Xbox-layout controller works alongside touch and keyboard, no setup needed — left stick to move/strafe, right stick to turn, RT to fire, A to use, RB to switch weapons, B to pause/back, Start to begin a challenge, Back/View to restart, D-pad for menu navigation.
...
- **Race a Friend**: host a room for whichever challenge is currently selected (from the challenge dialog's own "Race a Friend" button, or the panel's "Host a Race") and share the 5-character code, or join one — both players' engines start at the same synchronized instant and race live, with the opponent's kills/secrets/time shown in a second bar under your own. Each racer runs a fully independent copy of the game (their own device, their own instance), so nothing about a race changes how a solo run is judged or recorded — the loser's own attempt still resolves normally and can still be a personal best. First to *validly* finish (still passing the category's rules) wins. The relay is a small Azure Function (`api/negotiate`) that hands out a room-scoped access token for Azure Web PubSub; it only ever carries small progress/result messages, never game state.
- **Sharing**: after a run, the system share sheet where available, plus Text (`sms:`), Email (`mailto:`), and Copy. The link carries the challenge and your time (`?c=<id>&t=<tics>`), so the friend opens straight into it with "time to beat" showing.
