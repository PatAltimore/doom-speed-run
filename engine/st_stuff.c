...
// --- doom-speed-run patch ---
// Always returns 0 in the browser build; see speedrun.c.
int speedrun_cheats_enabled(void);
...
ST_Responder (event_t* ev)
{
  ...
  {
    // --- doom-speed-run patch: cheat codes disabled ---
    // Vanilla accepts iddqd/idkfa/idclev/... typed live at any time. A
    // timed competition can't allow that, so the whole cheat block is
    // gated on speedrun_cheats_enabled() (speedrun.c), which is always 0
    // in this build. The gate is a function rather than deleting the
    // code so the vanilla source stays diffable against upstream.
    if (!netgame && gameskill != sk_nightmare && speedrun_cheats_enabled())
    {
    ...
    // 'clev' change-level cheat (also gated -- see the doom-speed-run patch above)
    if (!netgame && speedrun_cheats_enabled() && cht_CheckCheat(&cheat_clev, ev->data2))
    {
      char		buf[3];
      int		epsd;
      int		map;
