  ...
  int 		damage )
{
    ...
    // --- doom-speed-run patch: category rule tracking ---
    // Pacifist and Tyson runs are judged on *who hurt what, with which
    // weapon*, and this is the one function every source of damage in
    // the game funnels through -- hitscan, melee, missiles, barrel and
    // rocket splash, telefrags. Observing here, after the same early
    // outs the real damage logic uses, is far simpler and more complete
    // than instrumenting each attack routine. See speedrun_on_damage.
#ifdef __EMSCRIPTEN__
    {
        extern void speedrun_on_damage(mobj_t *target, mobj_t *inflictor, mobj_t *source, int damage);
        speedrun_on_damage(target, inflictor, source, damage);
    }
#endif
