  ...
  skill_t	skill)
{
    ...
    // --- doom-speed-run patch: level (re)start ---
    // Every fresh attempt -- a new game, a warp, a reborn-after-death
    // reload -- comes through here, so this is where per-attempt rule
    // counters reset. See speedrun_on_level_start.
#ifdef __EMSCRIPTEN__
    {
        extern void speedrun_on_level_start(void);
        speedrun_on_level_start();
    }
#endif
