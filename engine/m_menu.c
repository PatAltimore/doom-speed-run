...
// --- doom-speed-run patch: tap-to-select menu items ---
...
// (via speedrun.c) would mean either re-declaring menu_t's exact layout
...
#ifdef __EMSCRIPTEN__
EMSCRIPTEN_KEEPALIVE int speedrun_menu_item_count(void)
{
...
EMSCRIPTEN_KEEPALIVE int speedrun_menu_base_y(void)
{
    return (menuactive && currentMenu) ? currentMenu->y : 0;
}

EMSCRIPTEN_KEEPALIVE void speedrun_menu_set_item(int index)
{
