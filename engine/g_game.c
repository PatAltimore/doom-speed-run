...
// --- doom-speed-run patch: twin-stick touch controls + run tracking ---
// The browser build (see speedrun.c) supports a twin-stick touch control
...
// web/shell.html). See speedrun_apply_touch_controls's own comment in
// speedrun.c for the scale factors and why forward/side/angleturn are
// passed by pointer instead of reusing mousex/mousey.
// speedrun_on_level_completed is the other hook this file needs: it's
// called from G_DoCompleted (below) so the browser side learns about a
// finished level -- and the exact leveltime it finished at -- the same
// tic the engine does, instead of trying to infer it from state polling.
...
// --- doom-speed-run patch: touch weapon switching ---
...
// skips needing one.
#ifdef __EMSCRIPTEN__
EMSCRIPTEN_KEEPALIVE void speedrun_switch_weapon(int direction)
{
...
{ 
    ...
    speedrun_apply_touch_controls(&forward, &side, &cmd->angleturn);
#endif
...
void G_DoCompleted (void)
{
    ...
    // --- doom-speed-run patch: level completed ---
    // leveltime is still this level's final tic count here (P_SetupLevel
    // zeroes it only when the *next* map loads), and secretexit still says
    // which exit was taken -- both are exactly what a speedrun result
    // needs, so snapshot them now. See speedrun_on_level_completed.
#ifdef __EMSCRIPTEN__
    speedrun_on_level_completed(secretexit ? 1 : 0);
#endif
